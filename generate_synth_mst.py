#!/usr/bin/env python3
"""Step 2: MST DP synthetic data at each epsilon × seed (HANDOFF grid)."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="MST synthetic data grid: eps × seeds on ACS train split."
    )
    p.add_argument(
        "--data-dir",
        type=Path,
    ...
    p.add_argument(
        ...
        type=float,
        default=0.1,
        help="Processing (binner) epsilon for MMPipeline.",
    )
    p.add_argument(
        "--delta",
        type=float,
    ...
    p.add_argument(
        ...
        type=int,
        default=-1,
        help="MST gen n_jobs (-1 = all cores).",
    )
