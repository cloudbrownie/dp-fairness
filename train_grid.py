...
Writes results/grid/eps{e}_seed{s}.csv (one row per intervention). A separate
merge step combines these into results/grid.csv.

Columns: eps, seed, intervention, accuracy, auc, dp_gap, eo_gap
"""
...
def run_condition(
    ...
    needs_dense = {"expgrad", "threshold"} & set(interventions)
    if needs_dense:
        logging.info("materializing dense matrix for fairlearn (%s)", ", ".join(sorted(needs_dense)))
    ...
    if "threshold" in to_run:
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            r = run_threshold(X_train_dense, y_train, A_train, X_test_dense, y_test, A_test)
